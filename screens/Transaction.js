...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (transactionType == "issue") {
      ...
      //toast para Android
      //ToastAndroid.show("Livro não encontrado, cheque o ID e tente novamente", ToastAndroid.LONG);
  ...
  checkStudentEligibilityForBookIssue = async (studentId) => {
    ...
    if (studentDoc.exists) {
      if (studentDoc.data().number_of_books_issued < 2) {
        ...
        //ToastAndroid.show("O aluno já atingiu o limite de 2 livros", ToastAndroid.LONG);
        Toast.show("O aluno já atingiu o limite de 2 livros", {
          duration: Toast.durations.LONG,
        });
        this.setState({
          bookId: "",
      ...
      isStudentEligible = false;
      //ToastAndroid.show("O ID do aluno não existe no banco de dados", ToastAndroid.LONG);
      Toast.show("O ID do aluno não existe no banco de dados", {
        duration: Toast.durations.LONG,
      });
    }
  ...
  checkStudentEligibilityForBookReturn = async (bookId, studentId) => {
    ...
    docs.forEach((doc) => {
      ...
      if (lastBookTransaction.student_id === studentId) {
        ...
        Toast.show("O livro não foi retirado por este aluno", {
          duration: Toast.durations.SHORT,
        });
      }
    });
